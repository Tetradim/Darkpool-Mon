...
APP_NAME="Darkpool Monitor"
DESKTOP_COMMAND_NAME="Darkpool Monitor.command"
ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
LOG_FILE="${HOME}/Desktop/Darkpool-Monitor.log"
BACKEND_PORT=8002
FRONTEND_PORT=3002
INSTALL_DEPS=0
NO_BROWSER=0
...
launch_app() {
  ...
  (cd "$ROOT_DIR" && "$venv_python" server.py) >> "$LOG_FILE" 2>&1 &
  pids+=("$!")

  log "Starting frontend on ${frontend_url}"
  (cd "$ROOT_DIR" && npm run dev -- --host 127.0.0.1 --port "$FRONTEND_PORT") >> "$LOG_FILE" 2>&1 &
  pids+=("$!")
  ...
  if ! wait_url "${backend_url}/health" 75; then
