...
Required source coverage can be satisfied by configuring `POLYGON_API_KEY` for price/NBBO and depth coverage, `NASDAQ_HALTS_RSS_ENABLED=true` for Nasdaq Trade Halt RSS halt/LULD coverage, and `SEC_EDGAR_USER_AGENT` for SEC EDGAR material-news context. When those adapters are missing, the confirmation plan recommends the specific required source family that needs to be added. These settings only mark confirmation adapters as configured; Sentinel confirmation and manual execution review are still required before any Pulse packet is prepared.
