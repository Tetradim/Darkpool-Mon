...
- `intent.risk_plan`: a planning envelope with estimated shares, max risk, stop, target, and planned notional. When sizing inputs are valid, blocked intents can still include this envelope for review; it is not an order and does not bypass Sentinel or Pulse guardrails.
